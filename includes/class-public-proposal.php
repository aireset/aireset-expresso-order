...
class EOP_Public_Proposal {
    ...
    private static function render_proposal_page( WC_Order $order ) {
        ...
        $is_flow_focus = $confirmed && $flow_enabled && in_array( $current_flow_stage, array( 'data', 'payment', 'contract', 'upload', 'products', 'completed' ), true );
        ...
        $customer_real_name = trim( $order->get_billing_first_name() . ' ' . $order->get_billing_last_name() );
        $has_customer_name  = '' !== $customer_real_name;
        $customer_name   = $customer_real_name ?: __( 'Nao informado', EOP_TEXT_DOMAIN );
    ...
    public static function render_admin_preview_markup( $settings = array() ) {
        ...
        $summary_eyebrow = trim( (string) ( $theme['summary_eyebrow'] ?: __( 'Contexto rápido', EOP_TEXT_DOMAIN ) ) );
        $summary_title = trim( (string) ( $theme['summary_title'] ?: __( 'Visão do pedido', EOP_TEXT_DOMAIN ) ) );
        ...
        $total_note = trim( (string) ( $theme['total_note'] ?: __( 'Assim que a etapa atual for concluída, o pedido segue para o time responsável.', EOP_TEXT_DOMAIN ) ) );
    ...
    private static function render_proposal_page_via_shared_renderer( $args ) {
        ...
        foreach ( $args['line_items'] as $line_item ) {
            ...
            // Lista de detalhes (label -> valor), organizada: preco un, desconto (% e valor), preco un com desconto.
            $specs = array();
            ...
            foreach ( $args['item_columns'] as $column ) {
                ...
                if ( 'quantity' === $key ) {
                    $specs[] = array( 'label' => $column['label'], 'value' => (string) $quantity );
                } elseif ( 'unit_price' === $key ) {
                    $specs[] = array( 'label' => $column['label'], 'value' => wp_strip_all_tags( wc_price( $line_item['unit_price'] ) ) );
                } elseif ( 'discount' === $key ) {
                    $percent = number_format_i18n( $line_item['discount_percent'], abs( $line_item['discount_percent'] - round( $line_item['discount_percent'] ) ) < 0.01 ? 0 : 2 );
                    $row     = array( 'label' => $column['label'], 'value' => $percent . '%', 'discount' => true );

                    if ( $line_item['discount_per_unit'] > 0 ) {
                        $row['sub'] = '- ' . wp_strip_all_tags( wc_price( $line_item['discount_per_unit'] ) ) . ' / ' . __( 'un.', EOP_TEXT_DOMAIN );
                    }

                    $specs[] = $row;
                } elseif ( 'discounted_unit_price' === $key ) {
                    $specs[] = array( 'label' => $column['label'], 'value' => wp_strip_all_tags( wc_price( $line_item['discounted_unit_price'] ) ) );
                }
            }

            if ( $args['show_sku'] && $product && $product->get_sku() ) {
                $specs[] = array( 'label' => __( 'SKU', EOP_TEXT_DOMAIN ), 'value' => $product->get_sku() );
            }
        ...
        if ( ! empty( $args['has_customer_name'] ) ) {
            $summary_rows[] = array( 'label' => $args['theme']['customer_label'], 'value' => $args['customer_name'] );
        }
        ...
        // Card "Visão do pedido" (contexto rápido) removido a pedido — dados do
        // cliente/pedido nao sao mais exibidos como card lateral na proposta publica.
        ...
        if ( ! $args['confirmed'] ) {
            ...
            if ( 'yes' === ( $args['theme']['actions_enabled'] ?? 'yes' ) && ! empty( $actions ) ) {
                $sidebar_cards[] = array(
                    'type'    => 'actions',
                    'eyebrow' => '' !== $args['actions_eyebrow'] ? $args['actions_eyebrow'] : __( 'Ações rápidas', EOP_TEXT_DOMAIN ),
                    'title'   => $args['actions_title'] ?: __( 'Ações rápidas', EOP_TEXT_DOMAIN ),
                    'actions' => $actions,
                    'note'    => '',
                );
            }
        ...
        $layout_context = array(
            ...
            'hero_meta_items' => array_values( array_filter( array(
                array( 'label' => $args['theme']['order_label'], 'value' => '#' . $args['order']->get_id() ),
                ! empty( $args['has_customer_name'] ) ? array( 'label' => $args['theme']['customer_label'], 'value' => $args['customer_name'] ) : null,
            ) ) ),
        ...
        return self::render_shared_proposal_markup(
            array(
                ...
                'sidebar_cards'   => array_values(
                    array_filter(
                        array(
                            null, // card "Visão do pedido" (contexto rápido) removido a pedido
...
<body>
    ...
    private static function get_shared_proposal_stylesheet() {
        return 'body .eop-login-wrap,body .eop-pdv,body .eop-proposal-wrap{border:0}.eop-proposal-wrap{max-width:var(--eop-preview-max-width,1120px);margin:32px auto;padding:0 16px 46px;font-family:var(--eop-preview-font-family,\'Segoe UI\',sans-serif);font-size:var(--eop-preview-text-size,16px);color:var(--eop-preview-text,#16243a);-webkit-text-size-adjust:100%;text-size-adjust:100%}.eop-proposal-card{display:grid;gap:24px}.eop-proposal-hero{position:relative;overflow:hidden;display:grid;grid-template-columns:minmax(0,1fr);gap:20px;padding:34px;border-radius:calc(var(--eop-preview-radius,18px) + 10px);background:var(--eop-preview-hero-bg,#0f1b35);box-shadow:0 28px 68px rgba(15,27,53,.24)}.eop-proposal-hero::before{content:\"\";position:absolute;inset:auto -10% -30% auto;width:340px;height:340px;border-radius:50%;background:radial-gradient(circle,var(--eop-preview-accent-glow,rgba(0, 255, 255,.28)),transparent 70%)}.eop-proposal-hero > *{position:relative;z-index:1}.eop-proposal-hero__main,.eop-proposal-hero__aside{display:grid;gap:18px;align-content:start}.eop-proposal-brandline{display:flex;align-items:flex-start;gap:18px}.eop-proposal-hero__bar{display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;margin-bottom:2px}.eop-proposal-brand{display:flex;align-items:center;justify-content:center;min-width:110px;max-width:210px;min-height:90px;padding:16px 18px;border-radius:26px;background:var(--eop-preview-brand-bg,rgba(255,255,255,.12));border:1px solid var(--eop-preview-border-soft,rgba(255,255,255,.18));backdrop-filter:blur(10px)}.eop-proposal-brand img,.eop-proposal-logo{display:block;width:100%;max-width:100%;max-height:60px;object-fit:contain}.eop-proposal-brand__fallback{color:var(--eop-preview-hero-text,#fff);font-size:13px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;text-align:center}.eop-proposal-hero__copy{display:grid;gap:12px;max-width:640px}.eop-proposal-hero__top{display:flex;flex-wrap:wrap;gap:10px}.eop-proposal-status,.eop-proposal-stage{display:inline-flex;align-items:center;min-height:38px;padding:0 14px;border-radius:999px;font-size:var(--eop-preview-chip-font-size,12px);font-weight:var(--eop-preview-chip-font-weight,800);letter-spacing:.08em;text-transform:uppercase}.eop-proposal-status{background:var(--eop-preview-hero-chip-bg,rgba(255,255,255,.16));color:var(--eop-preview-hero-chip-text,#fff);border:1px solid var(--eop-preview-border-soft,rgba(255,255,255,.18))}.eop-proposal-stage{background:var(--eop-preview-accent-soft,rgba(0, 255, 255,.22));color:var(--eop-preview-hero-chip-text,#fff);border:1px solid var(--eop-preview-accent-border,rgba(0, 255, 255,.28))}.eop-proposal-eyebrow{display:block;color:var(--eop-preview-hero-muted,rgba(255,255,255,.78));font-size:var(--eop-preview-hero-eyebrow-size,11px);font-weight:900;letter-spacing:.18em;text-transform:uppercase}.eop-proposal-title{margin:0;font-size:var(--eop-preview-title-size,46px);line-height:var(--eop-preview-hero-title-line-height,.98);font-weight:var(--eop-preview-hero-title-weight,800);letter-spacing:-.05em;color:var(--eop-preview-hero-text,#fff)}.eop-proposal-text{margin:0;max-width:58ch;color:var(--eop-preview-hero-muted,rgba(255,255,255,.78));font-size:var(--eop-preview-text-size,16px);line-height:var(--eop-preview-hero-text-line-height,1.7)}.eop-proposal-hero__context{display:flex;flex-wrap:wrap;gap:10px;margin-top:8px}.eop-proposal-ctx{display:inline-flex;align-items:center;gap:6px;padding:9px 14px;border-radius:14px;background:var(--eop-preview-brand-bg,rgba(255,255,255,.1));border:1px solid var(--eop-preview-border-soft,rgba(255,255,255,.18));color:var(--eop-preview-hero-muted,rgba(255,255,255,.78));font-size:13px;font-weight:600;font-variant-numeric:tabular-nums}.eop-proposal-ctx strong{color:var(--eop-preview-hero-text,#fff);font-weight:800}.eop-proposal-ctx--total{background:var(--eop-preview-accent-soft,rgba(0,255,255,.18));border-color:var(--eop-preview-accent-border,rgba(0,255,255,.28))}.eop-proposal-hero__aside{padding:24px;border-radius:28px;background:var(--eop-preview-side-bg,#f6f8fc);border:1px solid var(--eop-preview-border-soft,rgba(255,255,255,.18));backdrop-filter:blur(10px)}.eop-proposal-hero__aside-label{color:var(--eop-preview-muted,#66768d);display:block;font-size:11px;font-weight:900;letter-spacing:.16em;text-transform:uppercase}.eop-proposal-hero__aside strong{font-size:44px;line-height:.95;letter-spacing:-.06em;color:var(--eop-preview-text,#16243a)}.eop-proposal-hero__aside p{margin:0;color:var(--eop-preview-text,#16243a);line-height:1.65}.eop-proposal-hero__meta{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;margin-top:4px}.eop-proposal-hero__meta-item{padding:14px 16px;border-radius:20px;background:var(--eop-preview-panel-bg,#fff);border:1px solid var(--eop-preview-border-soft,rgba(255,255,255,.18))}.eop-proposal-hero__meta-item span{display:block;color:var(--eop-preview-muted,#66768d);font-size:10px;font-weight:900;letter-spacing:.14em;text-transform:uppercase}.eop-proposal-hero__meta-item strong{display:block;margin-top:7px;font-size:18px;line-height:1.2;color:var(--eop-preview-text,#16243a)}.eop-proposal-overview{display:grid;grid-template-columns:minmax(0,1.15fr) minmax(300px,.85fr);gap:24px;align-items:start}.eop-proposal-overview.is-single-column{grid-template-columns:minmax(0,1fr)}.eop-proposal-overview__main,.eop-proposal-overview__side{display:grid;gap:18px}.eop-proposal-overview__side{position:sticky;top:18px}.eop-proposal-section,.eop-proposal-summary-card{padding:24px;border-radius:28px;border:1px solid var(--eop-preview-border-soft,rgba(255,255,255,.18));box-shadow:0 18px 38px rgba(15,27,53,.08)}.eop-proposal-section{background:var(--eop-preview-panel-bg,#fff)}.eop-proposal-summary-card{background:var(--eop-preview-side-bg,#f6f8fc)}.eop-proposal-section__head{display:flex;justify-content:space-between;gap:12px;align-items:flex-end;margin-bottom:18px}.eop-proposal-section__eyebrow,.eop-proposal-summary-card__eyebrow{display:block;margin-bottom:6px;color:var(--eop-preview-muted,#66768d);font-size:11px;font-weight:900;letter-spacing:.14em;text-transform:uppercase}.eop-proposal-section__head h2,.eop-proposal-summary-card h2{margin:0;font-size:var(--eop-preview-section-title-size,26px);font-weight:var(--eop-preview-section-title-weight,800);line-height:1.06;letter-spacing:-.04em;color:var(--eop-preview-text,#16243a)}.eop-proposal-meta{display:grid;gap:12px}.eop-proposal-meta p{display:grid;gap:6px;margin:0;padding:14px 16px;border:1px solid var(--eop-preview-border-soft,rgba(255,255,255,.18));border-radius:18px;background:var(--eop-preview-panel-bg,#fff)}.eop-proposal-meta strong{font-size:12px;letter-spacing:.08em;text-transform:uppercase;color:var(--eop-preview-muted,#66768d)}.eop-proposal-items{display:grid;gap:14px}.eop-proposal-item{display:grid;grid-template-columns:108px minmax(0,1fr);gap:14px 18px;align-items:start;padding:18px;border:1px solid var(--eop-preview-border-soft,rgba(255,255,255,.18));border-radius:24px;background:var(--eop-preview-panel-bg,#fff)}.eop-proposal-item__media{width:108px;height:108px;border-radius:24px;overflow:hidden;background:var(--eop-preview-side-bg,#f6f8fc);border:1px solid var(--eop-preview-border-soft,rgba(255,255,255,.18));display:flex;align-items:center;justify-content:center;font-weight:800;font-size:20px;color:var(--eop-preview-muted,#66768d)}.eop-proposal-item__media img{display:block;width:100%;height:100%;object-fit:cover}.eop-proposal-item__body{min-width:0}.eop-proposal-item__name{margin:0 0 12px;font-size:var(--eop-preview-item-title-size,19px);font-weight:var(--eop-preview-item-title-weight,700);line-height:1.25;letter-spacing:-.01em;color:var(--eop-preview-text,#16243a)}.eop-proposal-item__meta{display:flex;flex-wrap:wrap;gap:8px 12px;color:var(--eop-preview-muted,#66768d);font-size:14px}.eop-proposal-item__pill{display:inline-flex;align-items:center;min-height:32px;padding:0 12px;border-radius:999px;background:var(--eop-preview-item-pill-bg,var(--eop-preview-accent-soft,rgba(0, 255, 255,.12)));color:var(--eop-preview-pill-text,#16243a);font-weight:700;line-height:1.2;white-space:nowrap;max-width:100%}.eop-proposal-item__pill--discount{display:grid;gap:2px;align-items:flex-start;padding:9px 12px;white-space:normal;border-radius:18px}.eop-proposal-item__pill-main{font-size:13px;line-height:1.2}.eop-proposal-item__pill-sub{font-size:12px;line-height:1.25;color:var(--eop-preview-muted,#66768d)}.eop-proposal-item__specs{display:grid;gap:0;margin-top:8px}.eop-proposal-item__spec{display:flex;justify-content:space-between;align-items:baseline;gap:16px;padding:9px 2px;border-bottom:1px solid rgba(15,27,53,.08)}.eop-proposal-item__spec:last-child{border-bottom:none;padding-bottom:0}.eop-proposal-item__spec dt{margin:0;color:var(--eop-preview-muted,#66768d);font-size:14px;font-weight:600}.eop-proposal-item__spec dd{margin:0;color:var(--eop-preview-text,#16243a);font-size:14px;font-weight:700;text-align:right}.eop-proposal-item__spec-sub{display:block;margin-top:1px;color:var(--eop-preview-muted,#66768d);font-size:12px;font-weight:500}.eop-proposal-item__spec--discount dd{color:#147a45}.eop-proposal-item__summary{grid-column:1 / -1;display:flex;justify-content:space-between;align-items:center;gap:14px;padding:14px 22px;border-radius:16px;background:rgba(0,3,75,.045);border:1px solid rgba(0,3,75,.05);margin-top:6px}.eop-proposal-item__summary-label{font-size:13px;color:var(--eop-preview-muted,#66768d);text-transform:uppercase;letter-spacing:.08em;font-weight:700}.eop-proposal-item__summary-price{display:flex;flex-direction:column;align-items:flex-end;gap:3px}.eop-proposal-item__summary-strike{color:var(--eop-preview-muted,#66768d);font-size:15px;font-weight:600;text-decoration:line-through;opacity:.85}.eop-proposal-item__summary strong{font-size:30px;line-height:1;color:var(--eop-preview-text,#16243a)}.eop-proposal-item__pill--save{background:#e7f7ee;color:#147a45}.eop-proposal-notes{padding:18px;border:1px solid var(--eop-preview-border-soft,rgba(255,255,255,.18));border-radius:22px;background:var(--eop-preview-panel-bg,#fff)}.eop-proposal-notes span{display:block;margin-bottom:8px;color:var(--eop-preview-muted,#66768d);font-size:13px;font-weight:700;letter-spacing:.06em;text-transform:uppercase}.eop-proposal-notes p{margin:0;color:var(--eop-preview-text,#16243a)}.eop-proposal-totals{display:grid;gap:2px}.eop-proposal-total{display:flex;justify-content:space-between;align-items:flex-start;gap:12px;padding:9px 0;color:var(--eop-preview-text,#16243a)}.eop-proposal-total.is-grand{font-size:20px;font-weight:800;border-top:2px solid var(--eop-preview-accent,#00ffff);margin-top:8px;padding-top:12px}.eop-proposal-total__value{display:grid;justify-items:end;gap:2px;text-align:right}.eop-proposal-total__value strong{font-size:15px;line-height:1.1;color:var(--eop-preview-text,#16243a)}.eop-proposal-total__value small{font-size:12px;line-height:1.25;color:var(--eop-preview-muted,#66768d)}.eop-proposal-button{display:inline-flex;align-items:center;justify-content:center;min-height:50px;padding:var(--eop-preview-button-padding,0 22px);border:none;border-radius:var(--eop-preview-button-radius,18px);background:var(--eop-preview-button-bg,var(--eop-preview-accent,#00ffff));color:var(--eop-preview-button-text,#fff);text-decoration:none;font-size:var(--eop-preview-button-font-size,16px);line-height:var(--eop-preview-button-line-height,1);font-weight:var(--eop-preview-button-font-weight,700);cursor:pointer;box-shadow:var(--eop-preview-button-shadow,0 16px 30px rgba(0, 255, 255,.2))}.eop-proposal-button--secondary{background:var(--eop-preview-secondary-button-bg,var(--eop-preview-side-bg,#f6f8fc));color:var(--eop-preview-secondary-button-text,var(--eop-preview-text,#16243a));box-shadow:none;border:1px solid var(--eop-preview-secondary-button-border,var(--eop-preview-border-soft,rgba(255,255,255,.18)));font-size:var(--eop-preview-secondary-button-font-size,var(--eop-preview-button-font-size,16px));line-height:var(--eop-preview-secondary-button-line-height,var(--eop-preview-button-line-height,1));font-weight:var(--eop-preview-secondary-button-font-weight,var(--eop-preview-button-font-weight,700));padding:var(--eop-preview-secondary-button-padding,var(--eop-preview-button-padding,0 22px));border-radius:var(--eop-preview-secondary-button-radius,var(--eop-preview-button-radius,18px))}.eop-proposal-button--pdf{background:var(--eop-preview-pdf-button-bg,#ffffff);color:var(--eop-preview-pdf-button-text,#16243a);box-shadow:var(--eop-preview-pdf-button-shadow,none);border:1px solid var(--eop-preview-pdf-button-border,var(--eop-preview-border-soft,rgba(255,255,255,.18)));font-size:var(--eop-preview-pdf-button-font-size,var(--eop-preview-button-font-size,16px));line-height:var(--eop-preview-pdf-button-line-height,var(--eop-preview-button-line-height,1));font-weight:var(--eop-preview-pdf-button-font-weight,var(--eop-preview-button-font-weight,700));padding:var(--eop-preview-pdf-button-padding,var(--eop-preview-button-padding,0 22px));border-radius:var(--eop-preview-pdf-button-radius,var(--eop-preview-button-radius,18px))}.eop-proposal-actions{display:flex;flex-wrap:wrap;gap:12px}.eop-proposal-actions form{display:flex;width:100%}.eop-proposal-actions .eop-proposal-button{width:100%}.eop-proposal-note{margin:0;padding:var(--eop-preview-alert-padding,14px 16px);border-radius:var(--eop-preview-alert-radius,18px);background:var(--eop-preview-alert-bg,#ecfdf5);border:1px solid var(--eop-preview-alert-border,#bbf7d0);color:var(--eop-preview-alert-text,#166534);font-size:var(--eop-preview-alert-font-size,15px);line-height:var(--eop-preview-alert-line-height,1.6)}.eop-proposal-wrap.is-flow-focus{max-width:var(--eop-preview-flow-focus-max-width,1040px);padding-bottom:34px}.eop-proposal-wrap.is-flow-focus .eop-proposal-card{gap:0}@media (max-width:980px){.eop-proposal-hero,.eop-proposal-overview{grid-template-columns:1fr}.eop-proposal-overview__side{position:static}.eop-proposal-actions .eop-proposal-button{width:100%}}@media (max-width:720px){.eop-proposal-wrap{font-size:15px;padding:0 12px 28px;margin:16px auto}.eop-proposal-card{gap:16px}.eop-proposal-hero{padding:22px;border-radius:22px;gap:18px}.eop-proposal-hero::before{display:none}.eop-proposal-brandline{gap:14px}.eop-proposal-brand{min-width:0;min-height:0;padding:12px 14px}.eop-proposal-brand img,.eop-proposal-logo{max-height:48px}.eop-proposal-hero__copy{gap:10px}.eop-proposal-title{font-size:var(--eop-preview-title-size-mobile,26px);letter-spacing:-.03em}.eop-proposal-text{font-size:15px;line-height:1.55}.eop-proposal-hero__aside{padding:18px;border-radius:20px;gap:12px}.eop-proposal-hero__aside strong{font-size:32px}.eop-proposal-hero__aside p{font-size:13px;line-height:1.5}.eop-proposal-hero__meta{grid-template-columns:1fr 1fr;gap:10px}.eop-proposal-hero__meta-item{padding:11px 13px}.eop-proposal-hero__meta-item strong{font-size:15px;margin-top:4px}.eop-proposal-section,.eop-proposal-summary-card{padding:18px;border-radius:20px}.eop-proposal-section__head{margin-bottom:14px}.eop-proposal-section__head h2,.eop-proposal-summary-card h2{font-size:20px}.eop-proposal-items{gap:10px}.eop-proposal-item{grid-template-columns:60px minmax(0,1fr);gap:13px;padding:13px;border-radius:16px;align-items:start}.eop-proposal-item__media{width:60px;height:60px;border-radius:14px;font-size:15px}.eop-proposal-item__name{font-size:16px;line-height:1.25;margin-bottom:6px}.eop-proposal-item__meta{gap:6px;font-size:13px}.eop-proposal-item__pill{min-height:25px;padding:0 10px;font-size:12px;font-weight:600}.eop-proposal-item__pill--discount{padding:5px 10px;border-radius:14px}.eop-proposal-item__pill-main{font-size:12px}.eop-proposal-item__specs{gap:6px}.eop-proposal-item__spec dt,.eop-proposal-item__spec dd{font-size:13px}.eop-proposal-item__spec-sub{font-size:11px}.eop-proposal-item__summary{grid-column:1 / -1;display:flex;flex-direction:row;justify-content:space-between;align-items:center;gap:12px;min-width:0;margin-top:10px;padding:14px 16px;text-align:left}.eop-proposal-item__summary-label{font-size:11px}.eop-proposal-item__summary-price{flex-direction:row;align-items:baseline;gap:8px}.eop-proposal-item__summary-strike{font-size:13px}.eop-proposal-item__summary strong{font-size:21px}.eop-proposal-meta p{padding:12px 14px}.eop-proposal-total{padding:8px 0}.eop-proposal-total.is-grand{font-size:18px}.eop-proposal-total__value strong{font-size:14px}.eop-proposal-actions{flex-direction:column;gap:10px}.eop-proposal-actions .eop-proposal-button{width:100%;min-height:52px}}@media (max-width:420px){.eop-proposal-hero__meta{grid-template-columns:1fr}.eop-proposal-title{font-size:24px}.eop-proposal-hero__aside strong{font-size:30px}}';
    }
                ...
                <?php else : ?>
                    <div class="eop-proposal-hero">
                        <div class="eop-proposal-hero__main">
                            <div class="eop-proposal-hero__bar">
                            ...
                            <div class="eop-proposal-hero__copy">
                                ...
                                <div class="eop-proposal-hero__context">
                                    <?php if ( ! empty( $context['total_value_html'] ) ) : ?><span class="eop-proposal-ctx eop-proposal-ctx--total"><?php echo esc_html( $context['total_label'] ?? '' ); ?> <strong><?php echo wp_kses_post( $context['total_value_html'] ); ?></strong></span><?php endif; ?>
                                    <?php foreach ( ( $context['hero_meta_items'] ?? array() ) as $meta_item ) : ?><span class="eop-proposal-ctx"><?php echo esc_html( $meta_item['label'] ?? '' ); ?> <strong><?php echo esc_html( $meta_item['value'] ?? '' ); ?></strong></span><?php endforeach; ?>
                                </div>
                            </div>
                        </div>
                    </div>
                                    ...
                                    <?php foreach ( ( $context['items'] ?? array() ) as $item ) : ?>
                                        <article class="eop-proposal-item">
                                            ...
                                            <div class="eop-proposal-item__body">
                                                ...
                                                <?php if ( ! empty( $item['pills'] ) ) : ?><div class="eop-proposal-item__meta"><?php foreach ( $item['pills'] as $pill ) : ?><span class="eop-proposal-item__pill<?php echo ! empty( $pill['save'] ) ? ' eop-proposal-item__pill--save' : ''; ?>"><?php echo esc_html( $pill['text'] ?? '' ); ?></span><?php endforeach; ?></div><?php endif; ?>
                                                <?php if ( ! empty( $item['specs'] ) ) : ?><dl class="eop-proposal-item__specs"><?php foreach ( $item['specs'] as $spec ) : ?><div class="eop-proposal-item__spec<?php echo ! empty( $spec['discount'] ) ? ' eop-proposal-item__spec--discount' : ''; ?>"><dt><?php echo esc_html( $spec['label'] ?? '' ); ?></dt><dd><?php echo esc_html( $spec['value'] ?? '' ); ?><?php if ( ! empty( $spec['sub'] ) ) : ?><span class="eop-proposal-item__spec-sub"><?php echo esc_html( $spec['sub'] ); ?></span><?php endif; ?></dd></div><?php endforeach; ?></dl><?php endif; ?>
                                            </div>
                                            <?php if ( ! empty( $item['summary_label'] ) || ! empty( $item['summary_value_html'] ) ) : ?><div class="eop-proposal-item__summary"><span class="eop-proposal-item__summary-label"><?php echo esc_html( $item['summary_label'] ?? '' ); ?></span><span class="eop-proposal-item__summary-price"><strong><?php echo wp_kses_post( $item['summary_value_html'] ?? '' ); ?></strong></span></div><?php endif; ?>
                                        </article>
                                    <?php endforeach; ?>
                                </div>
                            </section>
    ...
    private static function get_public_proposal_theme( $settings ) {
        ...
        return array(
            ...
            'accent_color'           => $resolve( 'customer_experience_accent_color', 'primary_color', '#00ffff' ),
            ...
            'title'                  => trim( (string) $resolve( 'customer_experience_title', 'proposal_title', 'Sua proposta está pronta para seguir' ) ),
            'description'            => trim( (string) $resolve( 'customer_experience_description', 'proposal_description', 'Confira os detalhes finais, valide os documentos e conclua a etapa atual em uma única jornada.' ) ),
            'total_label'            => trim( (string) $resolve( 'customer_experience_total_label', '', 'Investimento aprovado' ) ),
            'total_note'             => trim( (string) $resolve( 'customer_experience_total_note', '', 'Assim que a etapa atual for concluída, o pedido segue para o time responsável.' ) ),
            ...
            'summary_eyebrow'        => trim( (string) $resolve( 'customer_experience_summary_eyebrow', '', 'Contexto rápido' ) ),
            'summary_title'          => trim( (string) $resolve( 'customer_experience_summary_title', '', 'Visão do pedido' ) ),
            ...
            'actions_eyebrow'        => trim( (string) $resolve( 'customer_experience_actions_eyebrow', '', 'Próxima ação' ) ),
            ...
            'alert_note'             => self::normalize_customer_experience_copy( trim( (string) $resolve( 'customer_experience_alert_note', '', '' ) ), 'Este bloco simula a jornada pública com a mesma hierarquia visual usada pelo cliente.', '' ),
            'success_message'        => trim( (string) $resolve( 'customer_experience_success_message', '', 'Proposta confirmada com sucesso.' ) ),
            'notes_label'            => trim( (string) $resolve( 'customer_experience_notes_label', '', 'Observações' ) ),
            ...
            'pending_status_text'    => trim( (string) $resolve( 'customer_experience_pending_status_text', '', 'Aguardando confirmação' ) ),
            ...
            'meta_status_value_preview' => trim( (string) $resolve( 'customer_experience_meta_status_value_preview', '', 'Aguardando confirmação' ) ),
            'meta_deadline_label'    => trim( (string) $resolve( 'customer_experience_meta_deadline_label', '', 'Prazo' ) ),
            'meta_deadline_value_preview' => trim( (string) $resolve( 'customer_experience_meta_deadline_value_preview', '', 'Entrega em até 3 dias úteis' ) ),
            ...
            'item_title_size'        => $normalize_size( $resolve( 'customer_experience_item_title_size', '', '19px' ), '23px' ),
            ...
            'button_bg'              => trim( (string) $resolve( 'customer_experience_primary_button_background_color', '', $resolve( 'customer_experience_accent_color', 'primary_color', '#00ffff' ) ) ),
            ...
            'button_border_color'    => trim( (string) $resolve( 'customer_experience_button_border_color', '', $resolve( 'customer_experience_primary_button_background_color', '', '#00ffff' ) ) ),
            ...
            'button_shadow'          => trim( (string) $resolve( 'customer_experience_button_shadow', '', '0 16px 30px rgba(0, 255, 255, .20)' ) ),
