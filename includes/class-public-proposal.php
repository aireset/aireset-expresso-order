...
<body>
    ...
    private static function render_proposal_page_via_shared_renderer( $args ) {
        ...
        foreach ( $args['line_items'] as $line_item ) {
            ...
            $has_discount = $line_item['discount_percent'] > 0.001 && $line_item['unit_price'] > $line_item['discounted_unit_price'];
            $show_price   = in_array( 'unit_price', $column_keys, true ) || in_array( 'discounted_unit_price', $column_keys, true );
            $show_disc    = in_array( 'discount', $column_keys, true ) || in_array( 'discounted_unit_price', $column_keys, true );

            // Card enxuto: nome + qtd + economia (-%), com preco cheio riscado junto ao total.
            $pills = array();

            if ( in_array( 'quantity', $column_keys, true ) && $quantity > 0 ) {
                $pills[] = array( 'text' => sprintf( __( 'Qtd. %s', EOP_TEXT_DOMAIN ), $quantity ) );
            }

            if ( $show_disc && $has_discount ) {
                ...
                $pills[] = array( 'text' => '-' . $percent . '%', 'save' => true );
            }

            // Para quantidade > 1 mostramos o valor unitario final, que nao duplica o total.
            if ( $show_price && $quantity > 1 ) {
                $pills[] = array( 'text' => sprintf( __( '%s/un.', EOP_TEXT_DOMAIN ), wp_strip_all_tags( wc_price( $line_item['discounted_unit_price'] ) ) ) );
            }

            $summary_strike = '';

            if ( $args['show_line_total'] && $show_price && $has_discount ) {
                $original_line = $line_item['unit_price'] * $quantity;

                if ( $original_line > $line_item['line_total'] ) {
                    $summary_strike = wc_price( $original_line );
                }
            }
            ...
            $proposal_items[] = array(
                ...
                'summary_strike_html'=> $summary_strike,
                                ...
                                <div class="eop-proposal-items">
                                    <?php foreach ( ( $context['items'] ?? array() ) as $item ) : ?>
                                        <article class="eop-proposal-item">
                                            ...
                                            <?php if ( ! empty( $item['summary_label'] ) || ! empty( $item['summary_value_html'] ) ) : ?><div class="eop-proposal-item__summary"><span class="eop-proposal-item__summary-label"><?php echo esc_html( $item['summary_label'] ?? '' ); ?></span><span class="eop-proposal-item__summary-price"><?php if ( ! empty( $item['summary_strike_html'] ) ) : ?><s class="eop-proposal-item__summary-strike"><?php echo wp_kses_post( $item['summary_strike_html'] ); ?></s><?php endif; ?><strong><?php echo wp_kses_post( $item['summary_value_html'] ?? '' ); ?></strong></span></div><?php endif; ?>
                                        </article>
                                    <?php endforeach; ?>
                                </div>
                            </section>
